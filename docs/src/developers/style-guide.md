...
!!! info
    The style guide is always a work in progress, and not all UnifiedPseudopotentialFormat code
    follows the rules. When modifying UnifiedPseudopotentialFormat, please fix the style violations
...
UnifiedPseudopotentialFormat uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/UnifiedPseudopotentialFormat.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the UnifiedPseudopotentialFormat directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to UnifiedPseudopotentialFormat have
