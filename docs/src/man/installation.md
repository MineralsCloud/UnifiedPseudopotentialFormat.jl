...
[UnifiedPseudopotentialFormat](https://github.com/MineralsCloud/UnifiedPseudopotentialFormat.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("UnifiedPseudopotentialFormat")
   ```

3. Run

   ```julia-repl
   julia> using UnifiedPseudopotentialFormat
   ```
...
version of UnifiedPseudopotentialFormat, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/UnifiedPseudopotentialFormat.jl"
```
...
our [GitHub repository](https://github.com/MineralsCloud/UnifiedPseudopotentialFormat.jl)
for new releases.
Once we release a new version, you can update UnifiedPseudopotentialFormat by typing

```@repl
using Pkg
Pkg.update("UnifiedPseudopotentialFormat")
Pkg.gc()
```
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("UnifiedPseudopotentialFormat")

   julia> Pkg.gc()
   ```

2. Press `Ctrl+D` to quit the current session. Start a new Julia session and
   reinstall UnifiedPseudopotentialFormat.
